...
function getPathError(pubPath, compiler, url) {
  const { publicPath } = getPaths(pubPath, compiler, url);
  ...
  if (
    (localPrefix.hostname !== null &&
      ...
      !hostNameIsTheSame) ||
    (publicPath && hostNameIsTheSame && url.indexOf(publicPath) !== 0) ||
    (!urlObject.hostname &&
  ...
  const pathname = querystring.unescape(urlObject.pathname);

  if (pathname.indexOf('\0') !== -1) {
    return 400;
  }

  return UP_PATH_REGEXP.test(path.normalize(`./${pathname}`)) ? 403 : false;
}
...
module.exports = {
  getFilenameFromUrl(pubPath, compiler, url) {
    if (getPathError(pubPath, compiler, url)) {
      return false;
    }
